...
export async function middleware(req: NextRequest) {
    ...
    if(path.startsWith("/resume")){
        ...
        if(id){
            const uname=await axios.get(process.env.NEXT_PUBLIC_BACKEND_URL+"/api/v1/getUser/"+id)
            return NextResponse.redirect(new URL("/resume/"+uname.data.username, req.url));
        }
    }
}
