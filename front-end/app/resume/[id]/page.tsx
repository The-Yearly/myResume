...
export default function Home({ params }: { params: Promise<{ id: number }> }) {
  ...
  useEffect(() => {
    const fetchId = async () => {
      const res = await axios.get(
        process.env.NEXT_PUBLIC_BACKEND_URL + "/api/v1/getId/" + username,
      );
      setResume((prev) => ({
        ...prev,
        uid: res.data.uid,
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (resume.uid != 0) {
        const res = await axios.post(
          process.env.NEXT_PUBLIC_BACKEND_URL + "/api/v1/getProjects",
          { uid: resume.uid },
        );
        setResume((prev) => ({
          ...prev,
  ...
  useEffect(() => {
    const getHero = async () => {
      if (resume.uid != 0) {
        const res = await axios.post(
          process.env.NEXT_PUBLIC_BACKEND_URL + "/api/v1/getHero",
          { uid: resume.uid },
        );
  ...
  useEffect(() => {
    const getAbout = async () => {
      if (resume.uid != 0) {
        const res = await axios.post(
          process.env.NEXT_PUBLIC_BACKEND_URL + "/api/v1/getAbout",
          { uid: resume.uid },
        );
  ...
  if (
    resume.heroloaded &&
    resume.aboutloaded &&
    resume.projectsLoaded &&
    resume.skillsLoaded &&
    resume.experienceLoaded &&
    resume.educationLoaded &&
    resume.stylesLoaded &&
    resume.contactLoaded
  ) {
    return (
      <>
        <Navbar />
        <div className="flex flex-col justify-center items-center min-h-screen">
          {SelectedHero ? (
            <SelectedHero
              content={{
                hero: resume.hero?.hero || "Hello, I'm ...",
                subhero: resume.hero?.subhero || "A passionate .......",
              }}
              herocolors={resume.heroColors}
            />
          ) : (
            <p>Style Not Found</p>
          )}
          {SelectedAbout ? (
            <SelectedAbout
              content={{
                bio: resume.about?.about || "",
                image: resume.about?.image || "http://placebeard.it/250/250",
              }}
            />
          ) : (
            <p>Style Not Found</p>
          ...
          {SelectedSkills ? (
            <SelectedSkills skills={resume.skills} />
          ) : (
            <p>Style Not Found</p>
          )}
          {SelectedProjects ? (
            <SelectedProjects content={resume.projects} />
          ) : (
            <p>Style Not Found</p>
          )}
          <div className="grid w-full grid-cols-1 lg:grid-cols-4">
            {SelectedExp ? (
              <SelectedExp experence={resume.exp} />
            ) : (
              <p>Style Not Found</p>
            )}
            {SelectedEducation ? (
              <SelectedEducation education={resume.education} />
            ) : (
              <p>Style Not Found</p>
            )}
          </div>
