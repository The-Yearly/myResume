...
import { User2Icon,Settings2 ,Menu, X } from "lucide-react"
      ...
      <div className="flex fixed z-30 w-full backdrop-blur h-16 items-center justify-between border-b-1">
        ...
        <nav className="hidden md:flex gap-6">
         <Link
          ...
          <Link href={(logged.uid!=undefined&&logged.uid!=0)?`/resume/${logged.uid}`:"/authentication"}
            className="text-md hover:border-b-2 mr-5"
          >
            <User2Icon/>
          </Link>
        ...
        {isMenuOpen && (
          <div className="absolute top-16 left-0 right-0 bg-background border-b md:hidden">
            <div className="container py-4 flex flex-col gap-4">
              <Link
                href={(logged.uid!=undefined&&logged.uid!=0)?`/resume/${logged.uid}`:"/authentication"}
                className="relative flex left-1/12 py-2 text-lg hover:border-b-2 border-sky-500 rounded-sm mr-5">
                <User2Icon className="mr-4"/>myResume
              </Link>
              <Link
                href={"/myResume"}
                className="relative flex left-1/12 py-2 text-lg hover:border-b-2 border-sky-500 rounded-sm  mr-5">
