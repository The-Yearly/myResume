      ...
      if (newProject.title.length != 0) {
        try {
          if (editIndex == null && isNew == true) {
            const res = await axios.post(
              process.env.NEXT_PUBLIC_BACKEND_URL + "/api/v1/addProject",
              { project: newProject, uid: logged.uid, session: logged.session },
            );
            toast(res.data.message);
            setIsNew(false);
          } else {
            if (editIndex != null) {
              const res = await axios.post(
                process.env.NEXT_PUBLIC_BACKEND_URL + "/api/v1/updateProject",
                {
                  project: newProject,
                  uid: logged.uid,
                  session: logged.session,
  ...
  useEffect(() => {
    const DeleteProject = async () => {
      ...
      if (deleteProject != null) {
        try {
          const res = await axios.post(
            process.env.NEXT_PUBLIC_BACKEND_URL + "/api/v1/deleteProject",
            { del: deleteProject, uid: logged.uid, session: logged.session },
          );
          toast(res.data.message);
          setRefresh(!refresh);
        } catch (error) {
    ...
    return (
      <div className="p-4">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          ...
          {SelectedProject ? (
            <SelectedProject content={projects} />
          ) : (
            <p>Style not found.</p>
          )}
        </div>
