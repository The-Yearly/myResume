...
export default function AboutEditor() {
  const [about, setAbout] = useState<About>({
    about: "Session Dont Match My Guy",
    image: "",
    style: "1",
  });
    ...
    const sendData = async () => {
      if (data != null) {
        try {
          console.log(data);
          const res = await axios.post(
            process.env.NEXT_PUBLIC_BACKEND_URL + "/api/v1/setAbout",
            { about: data, uid: logged.uid, session: logged.session },
          );
    ...
    return (
      <div className="space-y-6">
          ...
          <form onSubmit={handleSubmit}>
            <div className="px-6 py-4 border-b border-gray-200">
              ...
              <p className="text-sm text-gray-500">
                Edit your about section content and profile image.
              </p>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          ...
          {SelectedAbout ? (
            <SelectedAbout
              content={{ bio: about.about, image: previewImage || about.image }}
            />
          ) : (
            <p>Style not found.</p>
          )}
