...
export default function ExperienceEdit() {
  ...
  const [submitselectedStyle, setSubmitSelectedStyle] =
    ...
    const fetchStyle = async () => {
      if (logged.uid != 0) {
        const res = await axios.post(
          ...
          { uid: logged.uid, session: logged.session },
        );
  ...
  useEffect(() => {
    const setStyle = async () => {
      ...
      if (submitselectedStyle != null) {
        const res = await axios.post(
          process.env.NEXT_PUBLIC_BACKEND_URL + "/api/v1/updateExperienceStyle",
          {
  ...
  useEffect(() => {
    const sendEd = async () => {
      if (deleteId != null) {
        setGotResp(true);
        const res = await axios.post(
          process.env.NEXT_PUBLIC_BACKEND_URL + "/api/v1/deleteExperience",
          { del: deleteId, uid: logged.uid, session: logged.session },
        );
        toast(res.data.message);
        setRefresh(!refresh);
        setGotResp(false);
  ...
  useEffect(() => {
    const getData = async () => {
      if (logged.uid != 0) {
        const res = await axios.post(
          process.env.NEXT_PUBLIC_BACKEND_URL + "/api/v1/getExperience",
          { uid: logged.uid, session: logged.session },
        );
      ...
      if (newExperience != null) {
        ...
        if (!isEditing) {
          res = await axios.post(
            process.env.NEXT_PUBLIC_BACKEND_URL + "/api/v1/addExperience",
            { exp: newExperience, uid: logged.uid, session: logged.session },
          );
          setNewExperience(null);
          toast(res.data.mesage);
        } else {
          res = await axios.post(
            process.env.NEXT_PUBLIC_BACKEND_URL + "/api/v1/updateExperience",
            { exp: newExperience, uid: logged.uid, session: logged.session },
          );
          setIsEditing(false);
          setNewExperience(null);
  ...
  const handleSaveExp = () => {
    ...
    if (isNewExp) {
      ...
      setExp((prev) =>
        prev.map((e) => (e.exid === currentExp.exid ? currentExp : e)),
      );
      setIsEditing(true);
      setNewExperience(currentExp);
    }
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-5">
        <div className="col-span-2">
          ...
          <div className="flex">
            ...
            <button
              disabled={gotResp}
              onClick={() => {
                setSubmitSelectedStyle({ exstyle: selectedStyle });
              }}
        ...
        <h1 className="text-2xl font-bold text-gray-800 col-span-2 md:col-span-1 flex items-center justify-center my-5">
          Manage Experience
        </h1>
        <div />
        <div className="flex justify-end items-center">
          <button
            ...
            className={`bg-sky-400 text-white hover:bg-sky-700 transition-colors hover:bg-sky-700e h-10 px-4 py-2 rounded flex items-center w-30 ${gotResp ? "cursor-wait" : "cursor-pointer"}`}
          >
          ...
          <div key={i} className="border rounded-lg shadow-sm p-6 bg-white">
            <div className="flex justify-between">
              <div>
                ...
                <p className="text-gray-600">{education.title}</p>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        ...
        {SelectedExp ? (
          <SelectedExp experence={experience} />
        ) : (
          <p>Style Not Found</p>
        )}
      </div>
