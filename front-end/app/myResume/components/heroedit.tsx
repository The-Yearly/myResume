...
export default function HeroEditor() {
  ...
  useEffect(() => {
    const setData = async () => {
      if (data != null) {
        const res = await axios.post(
          process.env.NEXT_PUBLIC_BACKEND_URL + "/api/v1/setHero",
          { hero: data, uid: logged.uid, session: logged.session },
        );
          ...
          <form onSubmit={handleSubmit}>
            <div className="px-6 py-4 border-b border-gray-200">
              ...
              <p className="text-sm text-gray-500">
                Edit your hero section content that appears at the top of your
                portfolio.
              </p>
            </div>
            ...
            <div className="flex px-6 py-4 border-t border-gray-200 justify-end space-x-4">
              ...
              <button
                type="submit"
                className="inline-flex items-center justify-center rounded-md font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 px-3 py-2 hover:text-white hover:bg-sky-600"
              >
                Save Changes
              </button>
            </div>
              ...
              <div className="bg-white border-1 max-w-[50vh] max-h-[50vh] p-6 rounded-xl w-full h-full overflow-y-auto">
                ...
                <div className="flex">
                  <div className="w-50">
                    <select
                      value={customize}
                      onChange={(e) => {
                        setCustomize(e.target.value as keyof heroColors);
                      }}
                      className="mt-1 block w-full max-w-xs rounded-md border border-gray-300 shadow-sm px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                    >
                      {Object.keys(heroColors).map((options) => (
                  ...
                  <div className="flex justify-center items-center w-50">
                    ...
                    <span
                      className={`w-5  h-5 ml-3 rounded-2xl border-1 bg-${tempHeroColors[customize]}`}
                    />
                  </div>
                </div>
                <div className="grid grid-cols-8 md:grid-cols-13">
                  {colors.map((color) => (
                    <button
                      key={color}
                      onClick={() => {
                        setTempHeroColors((prev) => ({
                          ...prev,
                          [customize]: color,
                        }));
                      }}
                      className={`w-5 mt-3 h-5 ml-3 rounded-2xl border-1 bg-${color}`}
                    />
                  ))}
                </div>
                <div className="flex justify-end gap-2 pt-2">
                  ...
                  <button
                    ...
                    onClick={() => {
                      setColorPickerModel(false);
                      setHeroColors(tempHeroColors);
                      console.log(tempHeroColors);
                    }}
