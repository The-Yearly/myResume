...
export default function EducationEditor() {
  ...
  const [currentEducation, setCurrentEducation] = useState<EducationI | null>(
    null,
  );
  ...
  const [currentDelEducation, setCurrentDelEducation] = useState<number | null>(
    null,
  );
  ...
  useEffect(() => {
    const getEd = async () => {
      if (logged.uid != 0) {
        const res = await axios.post(
          process.env.NEXT_PUBLIC_BACKEND_URL + "/api/v1/getEducations",
          { uid: logged.uid },
        );
  ...
  useEffect(() => {
    const setStyle = async () => {
      ...
      if (submitselectedStyle != null) {
        const res = await axios.post(
          process.env.NEXT_PUBLIC_BACKEND_URL + "/api/v1/updateEducationStyle",
          {
      ...
      if (newEd != null) {
        ...
        if (!isEditing) {
          res = await axios.post(
            process.env.NEXT_PUBLIC_BACKEND_URL + "/api/v1/addEducation",
            { ed: newEd, uid: logged.uid, session: logged.session },
          );
          setNewEd(null);
          toast(res.data.mesage);
        } else {
          res = await axios.post(
            process.env.NEXT_PUBLIC_BACKEND_URL + "/api/v1/updateEducation",
            { ed: newEd, uid: logged.uid, session: logged.session },
          );
          setIsEditing(false);
          setNewEd(null);
  ...
  useEffect(() => {
    const sendEd = async () => {
      if (currentDelEducation != null) {
        ...
        const res = await axios.post(
          process.env.NEXT_PUBLIC_BACKEND_URL + "/api/v1/delEducation",
          {
            del: currentDelEducation,
            uid: logged.uid,
            session: logged.session,
          },
        );
  ...
  const handleSaveEducation = () => {
    ...
    const isNewEducation = !educations.some(
      (e) => e.degree === currentEducation.degree,
    );
    console.log(currentEducation);
    if (isNewEducation) {
      ...
      setEducations((prev) =>
        prev.map((e) =>
          e.degree === currentEducation.degree ? currentEducation : e,
        ),
      );
      setNewEd(currentEducation);
    }
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-5">
        <div className="col-span-2 ">
          ...
          <div className="flex">
            ...
            <button
              disabled={gotResp}
              onClick={() => {
                setSubmitSelectedStyle({ estyle: selectedStyle });
              }}
        ...
        <h1 className="text-2xl col-span-2 md:col-span-1 flex items-center justify-center my-5 font-bold text-gray-800">
          Manage Education
        </h1>
        <div />
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        ...
        {SelectedEd ? (
          <SelectedEd education={educations} />
        ) : (
          <p>Style Not Found</p>
        )}
      </div>
